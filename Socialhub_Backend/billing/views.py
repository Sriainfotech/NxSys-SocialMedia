...
def _reset_post_usage(user):
    """Reset post usage counter when a user buys or renews a plan."""
    PostUsage.objects.filter(user=user).update(
        posts_used=0,
        daily_posts_used=0,
...
class VerifySubscriptionView(APIView):
    ...
    def post(self, request):
        ...
        _reset_post_usage(request.user)
