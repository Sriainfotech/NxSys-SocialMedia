...
from .models import OAuthState, Post, SocialAccount, PostingSchedule, Design, EmailVerification
...
from .throttles import (
    LoginRateThrottle, RegisterRateThrottle, TokenRefreshRateThrottle,
    SocialConnectRateThrottle, PostCreateRateThrottle, PasswordResetRateThrottle,
)
...
_ALLOWED_VIDEO_TYPES = {"video/mp4", "video/quicktime", "video/webm", "video/x-msvideo", "video/x-matroska", "video/m4v"}
_ALLOWED_IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".gif", ".webp"}
_ALLOWED_VIDEO_EXTENSIONS = {".mp4", ".mov", ".avi", ".mkv", ".webm", ".m4v"}
_MAX_IMAGE_SIZE = 10 * 1024 * 1024   # 10 MB
_MAX_VIDEO_SIZE = 200 * 1024 * 1024  # 200 MB


def _validate_upload(f):
    """Raise ValueError for disallowed file type or oversized file."""
    content_type = getattr(f, "content_type", "").lower()
    ext = os.path.splitext(f.name.lower())[1] if f.name else ""

    is_video = content_type in _ALLOWED_VIDEO_TYPES or ext in _ALLOWED_VIDEO_EXTENSIONS
    is_image = content_type in _ALLOWED_IMAGE_TYPES or ext in _ALLOWED_IMAGE_EXTENSIONS

    if not is_video and not is_image:
        raise ValueError(
            f"Unsupported file type '{content_type or ext}'. "
            "Allowed: JPEG, PNG, GIF, WEBP (images) or MP4, MOV, AVI, MKV, WEBM (videos)."
        )
    ...
    if is_video and size > _MAX_VIDEO_SIZE:
        raise ValueError(f"Video file too large ({size // (1024*1024)} MB). Maximum allowed: 200 MB.")
    if is_image and size > _MAX_IMAGE_SIZE:
        raise ValueError(f"Image file too large ({size // (1024*1024)} MB). Maximum allowed: 10 MB.")
...
class CSRFTokenView(APIView):
    """Call GET /api/csrf/ on app load. Django sets the csrftoken cookie
    (readable by JS — not HttpOnly) so the frontend can read it and send
...
class CreatePost(APIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            # Paginate first so we only load the current page into memory
            paginator = StandardResultsPagination()
            page = paginator.paginate_queryset(qs, request)
            page_posts = page if page is not None else list(qs)

            # N+1 fix: batch-fetch all social accounts referenced by this page
            all_account_ids = set()
            for p in page_posts:
                all_account_ids.update(p.target_accounts or [])
            accounts_map = (
                {acc.id: acc for acc in SocialAccount.objects.filter(
                    id__in=all_account_ids, user=request.user
                )}
                if all_account_ids else {}
            )

            serializer = PostSerializer(
                page_posts, many=True,
                context={"request": request, "accounts_map": accounts_map},
            )
            if page is not None:
...
class SocialAccountView(APIView):
    ...
    def get(self, request, pk=None):
        try:
            ...
            qs = SocialAccount.objects.filter(user=request.user).order_by("platform", "created_at")
            paginator = StandardResultsPagination()
            page = paginator.paginate_queryset(qs, request)
            if page is not None:
                return paginator.get_paginated_response(SocialAccountSerializer(page, many=True).data)
...
class SocialConnectStartView(APIView):
    permission_classes = [IsAuthenticated]
    throttle_classes = [SocialConnectRateThrottle]
...
class TokenRefreshCookieView(APIView):
    """Read the refresh token from the HttpOnly cookie, issue a new access
    cookie (and a new refresh cookie when ROTATE_REFRESH_TOKENS is True)."""
    permission_classes = [AllowAny]
    ...
    def post(self, request):
        ...
        _set_auth_cookies(response, access_token=serializer.validated_data["access"])

        new_refresh = serializer.validated_data.get("refresh")
        if new_refresh:
            _set_auth_cookies(response, refresh_token=new_refresh)
...
class HealthCheckView(APIView):
    ...
    Returns HTTP 200 when both the DB and cache are reachable, 503 if the DB
    is down (cache degradation is non-fatal and still returns 200 with a flag).
    """
    permission_classes = [AllowAny]
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            reset_url = (
                f"{getattr(settings, 'FRONTEND_URL', '')}/reset-password"
                f"?uid={uid}&token={token}"
            )
            send_mail(
                subject="Reset your NxSocial password",
                message=(
            ...
            logger.info("PasswordReset: sent reset email to user_id=%s", user.id)
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request):
        ...
        logger.info("PasswordReset: password changed for user_id=%s", user.id)
...
class SendVerificationEmailView(APIView):
    ...
    Sends (or resends) an email verification link to the authenticated user.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        verify_url = (
            f"{getattr(settings, 'FRONTEND_URL', '')}/verify-email?token={token}"
        )
        send_mail(
            subject="Verify your NxSocial email address",
            message=(
                f"Hello {user.username},\n\n"
        ...
        logger.info("EmailVerification: sent verification to user_id=%s", user.id)
...
class VerifyEmailView(APIView):
    ...
    def get(self, request):
        ...
        logger.info("EmailVerification: verified email for user_id=%s", ev.user_id)
