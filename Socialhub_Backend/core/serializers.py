...
class PostSerializer(serializers.ModelSerializer):
    ...
    def get_target_account_details(self, obj):
        ...
        When called from a list view, the view passes an ``accounts_map``
        dict in the serializer context (keyed by account id) to avoid an
        extra DB query per post (N+1).  Falls back to a direct query for
        single-object retrieval where no map is provided.
        """
        if not obj.target_accounts:
            return []

        accounts_map = self.context.get("accounts_map")
        if accounts_map is not None:
            return [
                ...
                for acc_id in obj.target_accounts
                if (acc := accounts_map.get(acc_id)) is not None
            ]

        # Fallback for single-post retrieval (no pre-fetched map)
        accounts = SocialAccount.objects.filter(id__in=obj.target_accounts, user=obj.user)
        return [
            {
                "id": acc.id,
                "platform": acc.platform,
