...
class PostManager(models.Manager):
    """Default manager — automatically excludes soft-deleted posts from every query."""
...
class Post(models.Model):
    ...
    # Filtered manager (default) excludes soft-deleted posts.
    objects = PostManager()
    # Unfiltered manager — use for admin, background tasks, or hard-deletes.
...
class EmailVerification(models.Model):
    """Stores the one-time token used to confirm a user's email address."""
    ...
    def __str__(self):
        return f"EmailVerification({self.user_id}, verified={self.is_verified})"
